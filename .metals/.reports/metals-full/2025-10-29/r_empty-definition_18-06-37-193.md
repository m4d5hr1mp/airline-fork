error id: file://<WORKSPACE>/airline-data/src/main/scala/com/patson/init/AirlineGenerator.scala:`<none>`.
file://<WORKSPACE>/airline-data/src/main/scala/com/patson/init/AirlineGenerator.scala
...
offset: 2277
uri: file://<WORKSPACE>/airline-data/src/main/scala/com/patson/init/AirlineGenerator.scala
text:
```scala
...
object AirlineGenerator extends App {
  ...
  def generateAirlines(count: Int) : Unit = {
    ...
    for (i <- 0 until count) {
      ...
      val user = User(userName = baseAirport.iata, email = "", Calendar.getInstance, Calendar.getInstance, UserStatus.ACTIVE, level = 0, None, List.empty)
      ...
      UserSource.setUserAirline(user, ne@@wAirline)
